...
void KeyingInterface::setKey(byte state)
{
  ...
  { // state == HIGH, KEY ON
    ...
    if (paddle.sendingMode == SEND_MODE_AUTO)
  ...
  if (state && (config.paddleTriggerPtt || paddle.sendingMode == SEND_MODE_AUTO))
  {
...
void KeyingInterface::checkPttTail()
{
  ...
  { // KEY is off
    ...
    { // PTT is on as result of KEY on
      ...
      if (paddle.sendingModeLast == SEND_MODE_PADDLE)
        hangTime = (12 * wordspaceLU * config.pttHangTimeWSPct / wpm);
...
void KeyingInterface::holdElementDuration(unsigned int length, int speed_wpm)
{
  ...
  while (((millis() - start) < ticks))
  {
    ...
    if (paddle.sendingMode == SEND_MODE_AUTO && paddle.wasTouched)
    {
