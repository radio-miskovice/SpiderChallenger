...
void PaddleInterface::handleInterrupt()
{
  if (paddleBreakIsPending)
  {
    ...
    awaitingRelease = paddleBreakIsPending;
  }
}
...
{ if( paddle.sendingMode == SEND_MODE_AUTO ) {
    if(!paddle.wasTouched) { // debounce check
      paddle.wasTouched = true;
      paddleState = PADDLE_PINS & PADDLE_INT_MASK;
